...
Value *VerifyBootloader(const char *name, State *state,
  ...
  int version = 0;
  if (bootloader.length() >= 5)
    version = int(bootloader[bootloader.length() - 5]);
...
void Register_librecovery_updater_sm8250() {
  RegisterFunction("sm8250.verify_bootloader_min", VerifyBootloader);
}
